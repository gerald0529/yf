...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        $("#grid").jqGrid({
            url: SITE_URL + "?ctl=Supplier_Manage&met=shopJoin&typ=json",
        ...
        $.dialog.confirm("此审核是审核用户的开店信息，只有审核通过，用户才能进行下一步付款，", function ()
        {
            Public.ajaxPost(SITE_URL + "?ctl=Supplier_Manage&met=editShopStatus&typ=json", {shop_id: t}, function (e)
            {
                if (e && 200 == e.status)
                {
                    ...
                    location.href= SITE_URL + "?ctl=Supplier_Manage&met=shopPay"; 
                }
                else
                {
...
$(function(){
	...
	$('.wrapper').on('click', '#reaudit', function (a) {
        ...
        var b = $('#grid').jqGrid('getGridParam', 'selarrrow'),
            c = b.join();
        return c ? void Public.ajaxPost('./index.php?ctl=Supplier_Manage&met=auditSupplierInfo&typ=json', {
            id: c
        }, function (a) {
