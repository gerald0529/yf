 ...
 */
class Payment_Alipay implements Payment_Interface
{
    public $notifyUrl;
    public $returnUrl;
    ...
    public function __construct($payment_row = array(), $order_row = array())
    {
        ...
        $this->notifyUrl = !empty($payment_row['notify_url']) ? $payment_row['notify_url'] : Yf_Registry::get('base_url') . "/paycenter/api/payment/alipay/notify_url.php";
        $this->returnUrl = Yf_Registry::get('base_url').'/paycenter/api/payment/alipay/return_url.php';
    }
...
EOF;
    ...
    public function getPayString($order_row)
    {
        require_once ROOT_PATH.'/libraries/Api/alipayMobile/AopSdk.php';
        $aop = new AopClient;
        $aop->gatewayUrl = $this->gatewayUrl;
        ...
        $aop->format = $this->format;
        $aop->charset = $this->postCharset;
        $aop->signType = $this->signType;
        $aop->alipayrsaPublicKey = $this->alipayPublicKey;

        $request = new AlipayTradeAppPayRequest();
        $requestJson = '{"product_code":"QUICK_MSECURITY_PAY","out_trade_no":"'.$order_row['union_order_id'].'","subject":"'.$order_row['trade_title'].'","total_amount":"'.$order_row['union_online_pay_amount'].'","body":"'.$order_row['trade_title'].'"}';
        $request->setNotifyUrl(Yf_Registry::get('base_url').'/paycenter/api/payment/alipay/notify_url.php'); //请填写您的异步通知地址
        $request->setBizContent($requestJson);
     ...
     * @access public
     */
    public function getNotifyData($notify_param)
    {
