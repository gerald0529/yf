...
class IndexCtl extends Yf_AppController
{
	...
	public function db()
	{
		...
		$db_row = current($db_cfg['db_cfg_rows']['master']['im-builder']);
		include $this->view->getView();
	}
	...
	public function initDbConfig()
	{
		...
		$file_admin = ROOT_PATH . '/imbuilder_admin/configs/db.ini.php';
	...
	public function install()
	{
		...
		else if (2 == $state_row['state'])
		{
			...
			foreach ($dir as $item)
			{
				...
				if (is_file($file))
				{
					$flag = $db->import($file, TABEL_PREFIX, 'im_');
...
function check_install_db()
{
	...
	if ($db_row = current($db_cfg['db_cfg_rows']['master']['im-builder']))
	{
