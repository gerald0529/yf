...
//        }
	...
	receiver	String	必选	接收者帐号，如果是个人，最大上限100人/次，如果是群组，仅支持1个；如果需要跨应用给个人发送信息，需要在接收者帐号前加appid和#。例如：appid=1，接收者帐号=a，
	则需要拼为1#a。由于群组ID为唯一ID，因此跨应用给群组发送消息无需增加appid和#。
	...
	function pushMsg($body_row)
	{
		...
		if (true || $this->BodyType == "json")
		{
			...
			/*$body_row = array();
			$body_row["pushType"] = "1";
			$body_row["appId"] = "8a48b55152114d54015215890cb907d5";
			$body_row["sender"] = "admin";
			$body_row["receiver"] = ["13761363453"];
			$body_row["msgType"] = "1";
		...
		// 发送请求
		$result = $this->curl_post($url, $body, $header);
        Yf_Log::log("result=".$result."\r\n url=".$url."\r\n"."body=".$body . json_encode($_REQUEST), Yf_Log::INFO, 'im_push');
		$this->showlog("response body = " . $result);
		if ($this->BodyType == "json")
		{ 
			//JSON格式
