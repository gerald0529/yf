    ...
    function uploadImage (uploadConfig){
        ...
        if ( $('.js-file-picker').length > 1 )
        {
            $(document).on('click', '.js-file-picker', function () {
                //当前list
                $list = $(this).prev().find('.uploader-list');
            });
        }
        ...
        uploader.on( 'fileQueued', function( file ) {
            var $li = $(
                    ...
                    '<i class="iconfont icon-cancel del-img remove-this del-por"></i></div>'
                ),
                $img = $li.find('img');
            ...
            if($("#fileList").children(".file-item").length == 5)
            {
                $(".filePicker").hide();
            }
        });
        ...
        //删除图片事件
        $list.on("click", ".del-img", function () {
