	...
	<div class="member_infor_content">
		<div class="div_head tabmenu clearfix">
			<ul class="tab clearfix">
				...
				<li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_Buyer_Directseller&met=directsellerOrder"><?=__('我的业绩')?></a></li>
        ...
        <div class="order_content">
			<div class="order_content_title clearfix">
				<form method="get" id="search_form" action="index.php" >
					...
					<p class="order_types">
						<a <?php if($status == ''):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_Buyer_Directseller&met=directsellerOrder"><?=__('全部订单')?></a>
						<a <?php if($status == 'wait_pay'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_Buyer_Directseller&met=directsellerOrder&status=wait_pay"><?=__('待付款')?></a>
						<a <?php if($status == 'wait_confirm_goods'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_Buyer_Directseller&met=directsellerOrder&status=wait_confirm_goods"><?=__('待收货')?></a>
						<a <?php if($status == 'finish'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_Buyer_Directseller&met=directsellerOrder&status=finish"><?=__('已完成')?></a>
						<a <?php if($status == 'cancel'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_Buyer_Directseller&met=directsellerOrder&status=cancel"><?=__('已取消')?></a>
					</p>
							...
							<?php foreach($val['goods_list'] as $ogkey=> $ogval):?>
								<tr class="tr_con">
									...
									<td class="td_color widt5">
										<?=number_format($ogval['directseller_commission_0'], 2, '.', '')?>
									</td>									 
								</tr>
							<?php endforeach;?>
							</table>
