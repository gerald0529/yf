            ...
            <ul>
                <?php foreach ($hot_sale as $hkey => $hval): ?>
                    <li>
                        ...
                        <p>
                            <a target="_blank" href="<?= Yf_Registry ::get('url') ?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?= ($hval['goods_id']) ?>" class="goods_list_name"><?= ($hval['common_name']) ?></a> <span><?= __('价格：') ?><strong><?= format_money($hval['common_price']) ?></strong></span> <a class="list_hot_intro bbc_btns" target="_blank" href="<?= Yf_Registry ::get('url') ?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?= ($hval['goods_id']) ?>"><?= __('立即抢购') ?></a>
                        </p>
                    </li>
                <?php endforeach; ?>
            </ul>
		...
		<span class="allresult">
			<?php if ($parent_cat_id) {
                if (count($parent_cat_id) == 1) { ?>
                    <a class="a_bold" href="<?php echo url("Goods_Goods/goodslist", ['cat_id' => $parent_cat_id[0]['cat_id']]); ?>"><?= ($parent_cat_id[0]['cat_name']) ?></a> <i class="iconfont icon-iconjiantouyou"></i> <span class="class_drap"><?= __('所有商品') ?></span>
                <?php } else {
                    foreach ($parent_cat_id as $key => $val) {
                        ?>
                        <?php if ($val['cat_parent_id'] == 0) { ?>
                            <a class="a_bold" href="<?php echo url("Goods_Goods/goodslist", ["cat_id" => $val['cat_id']]); ?>"><?= ($val['cat_name']) ?></a> <i class="iconfont icon-iconjiantouyou"></i>
                        <?php } else { ?>
							...
							<?php foreach ($val['silbing'] as $cckey => $ccval) { ?>
                                <a href="<?= url("Goods_Goods/goodslist", ["cat_id" => $ccval['cat_id']]) ?>"><?= ($ccval['cat_name']) ?></a>
                            <?php } ?>
							</p>
                                <?php } ?>
						</span>
            ...
            <div class="cl_brand_name">
                <ul class="clearfix">
                    <?php foreach ($brand_property['brand_list'] as $key => $brand_data) { ?>
                        <li class="clearfix">
                            <a href="<?= url("Goods_Goods/goodslist", ["cat_id" => $cat_id, "brand_id" => $brand_data['brand_id']]) ?>&<?= $brand_property['search_string'] ?>"> <img class='lazy' data-original="<?= $brand_data['brand_pic'] ?>" alt="<?= $brand_data['brand_name'] ?>"> </a> <span><?= $brand_data['brand_name'] ?></span>
                        </li>
                    <?php } ?>
                </ul>
            </div>
        ...
        <div class="cbrand class clearfix">
            ...
            <div class="cl_brand_name">
                <?php foreach ($brand_property['child_cat'] as $key => $child_cat_data) { ?>
                    <a href="<?= url("Goods_Goods/goodslist", ["cat_id" => $child_cat_data['cat_id']]) ?>"><?= $child_cat_data['cat_name']; ?></a>
                <?php } ?>
            </div>
        </div>
    <?php } ?>
            ...
            <div class="cbrand class clearfix">
                ...
                <div class="cl_brand_name">
                    <?php foreach ($property_data['property_values'] as $k => $property_value_data) { ?>
                        <a href="<?= url("Goods_Goods/goodslist", ["cat_id" => $cat_id]) ?>&property_id=<?= $property_data['property_id'] ?>&property_value_id=<?= $property_value_data['property_value_id'] ?>&<?= $brand_property['search_string'] ?>"><?= $property_value_data['property_value_name']; ?></a>
                    <?php } ?>
                </div>
            </div>
        <?php } ?>
                ...
                <ul class="cl_content_ul">
                    <?php foreach ($recommend_row as $rekey => $reval): ?>
                        <li>
                            ...
                            <a target="_blank" href="<?= url("Goods_Goods/goods", ["type" => "goods", "gid" => $reval['goods_id']]) ?>" class="cr_xie_lia1">
                                <div class="goodslist_img3"><img class='lazy' data-original="<?= image_thumb($reval['common_image'], 160, 160) ?>"/></div>
                                
                                <a class="cr_xie_span0" href="<?= url("Goods_Goods/goods", ["type" => "goods", "gid" => $reval['goods_id']]) ?>">
                                    <?php if (mb_strwidth($reval['common_name'], 'utf8') > 40) {
                                        echo $str = mb_strimwidth($reval['common_name'], 0, 40, '...', 'utf8');
                                    } else {
                                        echo $reval['common_name'];
                                    } ?>
                                </a> <span class="cr_xie_mon bbc_color">
        ...
        <div class="cr_head">
            <div class="crhead1 clearfix">
                <div class="crhead1_line">
                    <div class="<?php if ($act == '' || $act == 'all'): ?>crhead1_line_red bbc_bg<?php else: ?>crhead1_line_white<?php endif; ?>">
                        <a href="<?= url("Goods_Goods/goodslist", ['act' => 'all', 'cat_id' => request_int('cat_id'), 'actorder' => $next_order]) ?><?php if ($_GET['keywords']) {
                            echo '&keywords=' . $_GET['keywords'];
                        } ?>" title="<?php if ($actorder === 'desc') { ?><?= __('点击按上架时间升序') ?><?php } else { ?><?= __('点击按上架时间降序') ?><?php } ?>"><?= __('上架时间') ?><span id='all_icon_jiantou' class="iconfont <?php if ($actorder === 'asc' && $act === 'all') { ?>icon-iconjiantoushang<?php } else { ?>icon-iconjiantouxia<?php } ?>"></a>
                    </div>
                    <div class="<?php if ($act == 'sale'): ?>crhead1_line_red bbc_bg<?php else: ?>crhead1_line_white<?php endif; ?>">
                        <a href="<?= url("Goods_Goods/goodslist", ['act' => 'sale', 'cat_id' => request_int('cat_id'), 'actorder' => $next_order]) ?><?php if ($_GET['keywords']) {
                            echo '&keywords=' . $_GET['keywords'];
                        } ?>" title="<?php if ($actorder === 'desc') { ?><?= __('点击按销量升序') ?><?php } else { ?><?= __('点击按销量降序') ?><?php } ?>"><?= __('销量') ?><span id='sale_icon_jiantou' class="iconfont <?php if ($actorder === 'asc' && $act === 'sale') { ?>icon-iconjiantoushang<?php } else { ?>icon-iconjiantouxia<?php } ?>"></a>
                    </div>
                    <div class="<?php if ($act == 'price'): ?>crhead1_line_red bbc_bg<?php else: ?>crhead1_line_white<?php endif; ?>">
                        <a href="<?= url("Goods_Goods/goodslist", ['act' => 'price', 'cat_id' => request_int('cat_id'), 'actorder' => $next_order]) ?><?php if ($_GET['keywords']) {
                            echo '&keywords=' . $_GET['keywords'];
                        } ?>" title="<?php if ($actorder === 'desc') { ?><?= __('点击按价格升序') ?><?php } else { ?><?= __('点击按价格降序') ?><?php } ?>"><?= __('价格') ?><span id='price_icon_jiantou' class="iconfont <?php if ($actorder === 'asc' && $act === 'price') { ?>icon-iconjiantoushang<?php } else { ?>icon-iconjiantouxia<?php } ?>"></a>
                    </div>
                    <div class="<?php if ($act == 'evaluate'): ?>crhead1_line_red bbc_bg<?php else: ?>crhead1_line_white<?php endif; ?>">
                        <a href="<?= url("Goods_Goods/goodslist", ['act' => 'evaluate', 'cat_id' => request_int('cat_id'), 'actorder' => $next_order]) ?><?php if ($_GET['keywords']) {
                            echo '&keywords=' . $_GET['keywords'];
                        } ?>" title="<?php if ($actorder === 'desc') { ?><?= __('点击按评论数升序') ?><?php } else { ?><?= __('点击按评论数降序') ?><?php } ?>"><?= __('评论数') ?><span id='evaluate_icon_jiantou' class="iconfont <?php if ($actorder === 'asc' && $act === 'evaluate') { ?>icon-iconjiantoushang<?php } else { ?>icon-iconjiantouxia<?php } ?>"></a>
                    </div>
                </div>
            ...
            <div class="crhead2 clearfix">
                ...
                <div id="ncs-freight-selector" class="ncs-freight-select">
                    ...
                    <input type="checkbox" class="checkbox rel_top-1" <?php if ($op1 == 'havestock'){ ?>checked<?php } ?> name="op1"/> <label><?= __('仅显示有货') ?></label> <input type="checkbox" class="checkbox rel_top-1" <?php if ($op2 == 'active'){ ?>checked<?php } ?> name="op2"/> <label><?= __('仅显示折扣商品') ?></label>
        ...
        <ul class="cr_xie clearfix">
            <?php if ($data['items']):
                foreach ($data['items'] as $key => $val):
                    ?>
                    <?php if (isset($val['good']) && !empty($val['good'])) {
                    $id = 'gid=' . $val['goods_id'];
                    $urlArr = ['gid' => $val['goods_id']];
                    ?>
                    <li>
                        
                        <a target="_blank" href="<?= url("Goods_Goods/goods", ["type" => "goods"] + $urlArr) ?>" class="cr_xie_lia1">
                            <div class="goodslist_img1">
                                <img class='lazy' data-original="<?= image_thumb($val['common_image'], 220, 220) ?>"/>
                            </div>
                            <span class="cr_xie_mon bbc_color"><?= format_money($val['g_price'] ? $val['g_price'] : $val['common_price']) ?></span> <span class="cr_xie_amon"></span>
                            <!--3.2.0 团购图标 -->
                        ...
                        <div class="list_padd">
                            <a href="<?= url("Goods_Goods/goods", ["type" => "goods"] + $urlArr) ?>" target="_blank" class="cr_xie_name pb-0 two-overflow" title="<?= ($val['common_name']) ?>">
                            ...
                            <p class="mt9">
                                        <span class="cr_xie_argue">
                                            <?= __('已售：') ?>
                                            <a href="<?= url("Goods_Goods/goods", ["type" => "goods"] + $urlArr) ?>"><?= ($val['common_salenum']) ?></a>
                                            <?= __('件') ?>
                                        </span> <span class="cr_xie_argue">
                                            <?= __('评论数：') ?>
                                    <a href="<?= url("Goods_Goods/goods", ["type" => "goods"] + $urlArr) ?>">
                                                <?= ($val['common_evaluate']) ?>
                                            </a>
                                        </span>
                            </p>
                            <span class="shop_name">
                                        <?= ($val['shop_name']) ?>
                                <?php if ($val['shop_self_support']) { ?>
                                    <p class="bbc_btns"><?= __('平台自营') ?></p>
                                <?php } ?>
                                    </span> <a onclick="collectGoods(<?= (@$val['goods_id']) ?>, this)" class="cr_xie_lia2" id="coll_<?= (@$val['goods_id']) ?>"> <span class="cr_xie_lia2_span1 iconfont <?php if ($val['is_favorite']) { ?> icon-taoxinshi bbc_color <?php } else { ?> icon-icoheart <?php } ?>"></span> <span class="cr_xie_lia2_span2"><?= __('收藏') ?></span> </a>
                            <?php if (!$val['common_is_virtual'] && !($val['product_is_behalf_delivery'] == 1 && $val['common_parent_id'])) { ?>
                                <?php
                                $stock = 1;
                                foreach ($val['good'] as $g) {
                                    if ($g['goods_stock'] == 0) {
                                        $stock = 0;
                                    }
                                }
                                ?>
                                <a class="cr_xie_lia2" onclick="addCart(<?= (@$val['goods_id']) ?>,<?= (@$val['shop_owner']) ?>,<?= (@$val['dist_owner']) ?>,<?= (@$val['common_stock']) ?>,<?= (@$stock) ?>)"> <span class="cr_xie_lia2_span1 cr_xie_lia2_span1—3 iconfont icon-wodegouwuche bbc_color"></span> <span class="cr_xie_lia2_span2 cr_xie_lia2_span2-1 "><?= __('加入购物车') ?></span> </a>
                            <?php } else { ?>
                                <?php if ($val['product_is_behalf_delivery'] == 1 && $val['common_parent_id']) { ?>
                                    <a class="cr_xie_lia2" onclick="BuyDistribution(<?= (@$val['goods_id']) ?>,<?= (@$val['shop_owner']) ?>,<?= (@$val['dist_owner']) ?>)"> <span class="cr_xie_lia2_span1 cr_xie_lia2_span1—3"></span> <span class="cr_xie_lia2_span2 cr_xie_lia2_span2-1"><?= __('立即购买') ?></span> </a>
                                <?php } else { ?>
                                    <a class="cr_xie_lia2" onclick="nowBuy(<?= (@$val['goods_id']) ?>,<?= (@$val['shop_owner']) ?>,<?= (@$val['dist_owner']) ?>)"> <span class="cr_xie_lia2_span1 cr_xie_lia2_span1—3"></span> <span class="cr_xie_lia2_span2 cr_xie_lia2_span2-1"><?= __('立即购买') ?></span> </a>
                                <?php } ?>
                            <?php } ?>
    ...
    window.addCart = function (e, i, j, f, g) {
        if ($.cookie("key")) {
            ...
            if (i == 1 || j == 1 || f == 0 || g == 0) {
                ...
                if (g == 0) {
                    Public.tips.warning('<?=__('部分规格缺货，请进入详情购买！')?>');
                    
                }
                if (f == 0) {
                    Public.tips.warning('<?=__('该商品已售空！')?>');
                }
            } else {
                $.ajax({
