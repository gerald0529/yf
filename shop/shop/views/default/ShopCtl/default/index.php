                ...
                <div class="sub">
                    ...
                    <div class="bbc-info_reset">
                        ...
                        <div class="content_reset">
                            ...
                            <div class="btns"><a href="index.php?ctl=Shop&met=goodsList&id=<?=$shop_id?>" class="goto"><?=__('进店逛逛')?></a><a href="javascript:;" onclick="collectShop(<?=$shop_id?>)"><?=__('收藏店铺')?></a></div>
                            <?php if(!empty($shop_all_base)){?>
                            <?php if($shop_all_base['shop_business'] == 1){?>
                            <dl class="no-border">
                                <dt><?=__('公司名称')?>：</dt>
                            ...
                            <?php }else{ ?>
                                <dl class="no-border">
                                    <dt><?=__('店家名称')?>：</dt>
                                    <dd><?=$shop_all_base['user_name']?></dd>
                                </dl>
                                <dl>
                                    <dt><?=__('电话')?>：</dt>
                                    <dd><?=$shop_all_base['contacts_phone']?></dd>
                                </dl>
                             <?php } ?>
                            <dl>
                                <dt><?=__('所在地')?>：</dt>
...
<?php if(empty($shop_base['is_renovation'])){?>
  ...
  <div class="clearfix">
      ...
      <div class="div_shop_Carouselfigure1" style="width:1200px;height:150px;overflow: hidden;background:#fff;">
      <?php if(!empty($shop_base['shop_banner']) ){ ?>
      <img src="<?=$shop_base['shop_banner']?>" width="1200px" height="150px;"/></a>
      <?php }else{ ?>
      <img src="<?= $this->view->img ?>/shop_img.png" width="1200px" /></a>
     ...
     <div id="nav" class="bbc-nav">
		<ul>     
          <li class="active9"><a href="index.php?ctl=Shop&id=<?=$shop_id?>"><span><?=__('店铺首页')?><i></i></span></a></li> 
         <?php 
			if($shop_nav['items'])
			{
                foreach ($shop_nav['items'] as $key => $value) 
 ...
 <div class="clearfix">
  ...
  <div class="t_goods_bot clearfix">
    <div class="t_goods_bot_left">
      <div class="goods_classify">
        ...
        <p class="classify_ser">
          <input  name="searchGoodsList" type="text" value="<?=__('搜索店内商品')?>" onclick="this.value = '';">
    ...
    <div id="hot_sales_list" class="ncs-top-panel">
          ...
          <li>
          <dl>
            <dt><a href="index.php?ctl=<?=$ctl?>&met=goods&gid=<?=$value['goods_id']?>"><?=$value['common_name']?></a></dt>
            <dd class="goods-pic"><a href="index.php?ctl=<?=$ctl?>&met=goods&gid=<?=$value['goods_id']?>"><span class="thumb size40"><i></i><img src="<?=$value['common_image']?>"  style="width:40px;height: 40px;"></span></a>
            ...
            <dd class="selled pngFix"><?=__('售出')?>：<strong class="num-color"><?=$value['common_salenum']?></strong><?=__('笔')?></dd>
          </dl>
        </li>
                <?php } }?>
                ...
                if (isset($hot_groupbuy_data)):
                    foreach ($hot_groupbuy_data as $key_hot => $value_hot):
                        ...
                        <li>
                            ...
                            <p class="current_hot_price bbc_color"><span class="common-color"><?=format_money($value_hot['groupbuy_price']) ?></span></p>
      ...
      <div class="bbc-main-container">
        ...
        <div class="content_s bbc-goods-list">
          <ul>
            ...
            <li>
              <dl>
                  <dt><a href="index.php?ctl=<?=$ctl?>&met=goods&gid=<?=$value['goods_id']?>" class="goods-thumb" target="_blank"><img  src="<?=image_thumb($value['common_image'],220,220)?>" alt="<?=$value['common_name']?>"></a>
                  <!--<ul class="goods-thumb-scroll-show">
                      <li class="selectedSS"><a href="javascript:void(0);"><img width="60" src="<?=$value['common_image']?>"></a></li>
                  </ul>-->
                </dt>
                <dd class="goods-name"><a href="index.php?ctl=<?=$ctl?>&met=goods&gid=<?=$value['goods_id']?>" title="<?=$value['common_name']?>" target="_blank"><?=$value['common_name']?></a></dd>
                <dd class="goods-info"><span class="priceSS bbc_color"><?=format_money($value['common_price']) ?></span> <span class="goods-sold"><?=__('已售')?>：<strong class="num-color"><?=$value['common_salenum']?></strong> <?=__('件')?></span></dd>
              </dl>
            </li>
              <?php }}?>
          </ul>
