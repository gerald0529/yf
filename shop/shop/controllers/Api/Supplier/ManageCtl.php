...
//		);
	...
	public function categoryStatus()
	{
		...
		if ($shop_class_bind_id)
		{
			...
			if (!$pass) {
				...
				$class_status['shop_class_bind_enable'] = 0; //拒绝
				$class_status['shop_class_bind_desc'] = $refusal_reason; //拒绝理由
			}
	...
	public function distributor_order()
	{
		...
		$dist_common_ids = array_column($dist_common,'common_id');//分销商品id
		$dist_order_goods  = $Order_GoodsModel->getByWhere(array('common_id:IN' => $dist_common_ids));
		$dist_orders    = array_unique(array_column($dist_order_goods,'order_id'));//含有分销商品的订单id
