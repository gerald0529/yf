	...
	public function index()
	{
		...
		if (Perm::checkUserPerm())
		{
			...
			if ($shop_info && $shop_info['shop_type'] == 2)
			{
				...
				{
                    location_to(Yf_Registry::get('url')."?ctl=Seller_Index&met=index&typ=e");
				}
			}else if($chain_rows){
            ...
            if(isset($shop_info['shop_business']) && $shop_info['shop_business'] == 1){
               ...
               $apply = 1;   
            }else if(!isset($shop_info['shop_business']) && Web_ConfigModel::value('supplier_type') == 1){
                $apply = 2;
            }else if(!isset($shop_info['shop_business']) && Web_ConfigModel::value('supplier_type') == 2){
                $apply = 1;
            }else{
            ...
            if($apply == 1){
                ...
                $shop_business = 1;
                $shop_help     = $ShopHelpModel->getByWhere(array('page_show:IN'=>array(3,4)),array('help_sort' => "asc"));
            }
...
//                }
	...
	public function editShopBase()
	{
        ...
        //名称不超过15个字符
        if(mb_strlen($shop_list['shop_name'],'utf8') > 15){
            return $this->data->addBody(-140, array(mb_strlen($shop_list['shop_name'])), __('店铺名称不能超过15个字符'), 250);
        }
    ...
    private function _getStep($shop_step_info){
        ...
        $url = Yf_Registry::get('url').'/index.php?ctl=Seller_Supplier_Settled&met=index&op='.$op.'&apply='.$apply;
        ...
        switch ($shop_step_info['rp']){
            ...
            case 'step1':
                ...
                if($shop_step_info['shop_status'] == '-1'){
                    if(Web_ConfigModel::value('supplier_type') == 3){
                        $op = 'step0';
                    }else{
                        $op = 'step1';
                    }
