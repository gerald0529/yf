...
class Api_Erp_OrderCtl extends Api_Controller{
    ...
    public function downOrder()
    {
        ...
        $Order_BaseModel->sql->setWhere('order_is_virtual',0);//虚拟订单
        $Order_BaseModel->sql->setWhere('chain_id',0);//门店自提订单

        $Goods_CommonModel = new Goods_CommonModel();
        $goods_common = $Goods_CommonModel->getByWhere(['product_is_behalf_delivery'=>1,'supply_shop_id:!='=>0]);
        $goods_common_id = array_column($goods_common,'common_id');

        $Order_GoodsModel = new Order_GoodsModel();
        $order_goods = $Order_GoodsModel->getByWhere(['common_id:IN'=>$goods_common_id]);

        $order_id = array_unique(array_column($order_goods,'order_id'));

        if(!empty($order_id)){
            $Order_BaseModel->sql->setWhere('order_id',$order_id,'NOT IN');
        }
        ...
        if($Order_Base){
            foreach($Order_Base as $key=>$value){
                ...
                if($value['order_receiver_address']){
                    $order_delivery_address = explode(' ',ltrim($value['order_receiver_address']));
...
//下载退款订单
    ...
    public function updateOrderState()
    {
        ...
        if ($operation == 'audit') {
            ...
            $order_number = request_string('order_id');
            $order_goods_id = request_string('product_id');
            $order_return = $orderReturnModel->getByWhere(array('order_number'=>$order_number));
            if($order_return){
                foreach($order_return as $k=>$v){
                    $order_return_id = $v['order_return_id'];
                    $return_shop_message = $v['return_reason'];
